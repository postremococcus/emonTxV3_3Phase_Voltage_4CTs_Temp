/*
emonTX V3 - 3-phase, 4CTs and Temperature (TT)

Combines Temperature routines from default "emonTxV3 Discrete Sampling v1.3" example code with:
-----------------------------------------------------------------------------------------------
...
#define PHASE3 15                                //  Number  of samples delay for L3, also size of array
...
const int networkGroup = 208;                    //  emonTx RFM12B wireless network group
...
double Vcal = 228.0;                           // Calibration constant for voltage input
...
double Phasecal2 = 0.57;                         // Calibration constant for phase shift L2
double Phasecal3 = 0.97;                         // Calibration constant for phase shift L3
...
typedef struct { int power1, power2, power3, power4, Vrms, temp; } PayloadTX;         // neat way of packaging data for RF comms
                                                                                     ...
                                                                                     // Added "temp" variable for temperature (TT)
...
int numSensors;                                  //addresses of sensors, MAX 4!!  
...
void setup() 
{
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------
  // Setup and check for presence of DS18B20
  //-------------------------------------------------------------------------------------------------------------------------------------------
  pinMode(DS18B20_PWR, OUTPUT);                    // Temperature (TT)
...
{ 
  ...
  if (DS18B20_STATUS==1)
  {
    ...
    {
      emontx.temp=(temp*10);                       //if reading is within range for the sensor convert float to int ready to send via RF
    }
  }
  ...
  // Removing these print statements is recommended for normal use (if not required).
    ...
    if (DS18B20_STATUS==1)                         // If there are temperature readings (TT)
    {
      Serial.print(" Temperature: "); Serial.println(emontx.temp*0.1); 
    }
